...
- **Commercial List (WC Main Premium)** - Commercial watchlist management with record profile operations (Watchlist → Commercial list → WC Main Premium → Find clean records → Suppress/Enrich/Edit operations). Includes Active, Deleted, and Suppressed/Enriched tab filter validation against MongoDB.
...
- **Global Search** - FacctList global search functionality (Sidebar → Search → Enter query → Filter by entity type/list type/status → View results → Open record profile). Supports search by record ID or name, with pagination and result filtering.
- **Suppressed and Enriched** - View and manage all suppressed/enriched record overrides (Watchlist → Suppressed and Enriched). Supports tabs (All, Suppressed, Enriched, Pending, Active, Expired), filtering by list name/record type/action type/status/tag/reason/review period, search by record ID, record detail drawer with release action, and pagination.
...
- **Data Export** - Data export management module (FacctList → Data Export). Supports Templates and Destination Config sections for configuring and managing watchlist data exports.
- **OFAC NON-SDN Profile Verification** - Profile view field mapping verification for OFAC NON-SDN records (Watchlist → Regulatory List → OFAC NON SDN → Search record → Open profile view → Verify mapped fields). Validates XML source → DB → UI field mapping. Includes entity-specific verification scenarios (e.g., entity 15268 BANK OF KUNLUN CO LTD), multi-name entity verification, and UI mapping gap detection (compares visible UI fields against MongoDB document fields to identify unmapped fields). Uses `@org:facctum` organization.
- **OFAC NON-SDN Data Mapping Verification** - Comprehensive data integrity verification across the full ingestion pipeline (XML → DB → UI) for OFAC NON-SDN regulatory list records. Three verification layers: (1) XML to DB mapping - verifies XML source fields are correctly stored in MongoDB including name types, ID types, date formats, and address mappings; (2) Profile View UI - verifies DB fields are correctly rendered on the UI profile view across Primary Details and Additional Details tabs; (3) UI Gap Analysis - identifies DB fields not displayed on the UI. Also validates the Excel mapping sheet for correctness and completeness. Uses `@org:datavium` organization. Tags: `@OFACNonSDNMapping @RegulatoryList`.
